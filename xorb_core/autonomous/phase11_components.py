...
class TemporalSignalPatternDetector:
    ...
    async def detect_patterns(self, signals: list['ThreatSignal']) -> dict[str, Any]:
        ...
        try:
            ...
            for i, (signal, label) in enumerate(zip(recent_signals, cluster_labels, strict=False)):
...
class RoleAllocator:
    ...
    async def _allocate_by_performance(self, agents: list['BaseAgent'], required_roles: list['RoleType'],
        ...
        from ..autonomous.intelligent_orchestrator import (
            AgentRole,  # Import here to avoid circular import
        )
...
class ConflictDetector:
    ...
    async def _check_response_conflict(self, response1: dict[str, Any], response2: dict[str, Any]) -> dict[str, Any] | None:
