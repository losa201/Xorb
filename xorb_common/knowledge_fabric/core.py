...
class KnowledgeFabric:
    ...
    async def analyze_knowledge_gaps(self, atom_type: Optional[AtomType] = None) -> Dict[str, Any]:
        ...
        expired_atoms = len([a for a in atoms if a.expires_at and a.expires_at < datetime.now(timezone.utc)])
        ...
        async with self.async_session() as session:
            query = select(AtomModel).where(
                and_(
                    ...
                    or_(AtomModel.expires_at.is_(None), AtomModel.expires_at > datetime.now(timezone.utc))
                )
        ...
        async with self.async_session() as session:
            query = select(AtomModel).where(
                and_(
                    ...
                    AtomModel.expires_at < datetime.now(timezone.utc)
                )
            )
            result = await session.execute(query)
