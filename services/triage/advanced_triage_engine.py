...
class TriageResult:
    ...
    severity_adjusted: SeverityLevel | None
...
class EmbeddingCache:
    ...
    async def get_embedding(self, text: str, text_hash: str) -> np.ndarray | None:
...
class AdvancedTriageEngine:
    ...
    async def find_similar_vulnerabilities(
        ...
        try:
            with faiss_search_duration.time():
                ...
                similar_findings = []
                for i, (similarity, idx) in enumerate(zip(similarities[0], indices[0], strict=False)):
    ...
    def build_triage_reasoning(
        ...
        is_duplicate: bool,
        confidence: float,
        adjusted_severity: SeverityLevel | None,
        priority_score: float
    ) -> str:
