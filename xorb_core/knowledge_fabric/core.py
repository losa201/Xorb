...
from .atom import AtomType, ConfidenceLevel, KnowledgeAtom, Source, ValidationResult
...
class KnowledgeFabric:
    ...
    async def get_atom(self, atom_id: str) -> KnowledgeAtom | None:
    ...
    async def update_atom(self, atom_id: str, updates: dict[str, Any], source: Source | None = None) -> bool:
    ...
    async def analyze_knowledge_gaps(self, atom_type: AtomType | None = None) -> dict[str, Any]:
        async with self.async_session() as session:
        ...
        expired_atoms = len([a for a in atoms if a.expires_at and a.expires_at < datetime.now(UTC)])
        ...
        async with self.async_session() as session:
            query = select(AtomModel).where(
                and_(
                    ...
                    or_(AtomModel.expires_at.is_(None), AtomModel.expires_at > datetime.now(UTC))
                )
        ...
        async with self.async_session() as session:
            query = select(AtomModel).where(
                and_(
                    ...
                    AtomModel.expires_at < datetime.now(UTC)
                )
            )
            result = await session.execute(query)
    ...
    async def _get_atom_hot(self, atom_id: str) -> KnowledgeAtom | None:
    ...
    async def _get_atom_warm(self, atom_id: str) -> KnowledgeAtom | None:
        async with self.async_session() as session:
    ...
    async def _search_hot_cache(self, query: str | None, atom_type: AtomType | None, tags: set[str] | None, min_confidence: float) -> list[KnowledgeAtom]:
        results = []
    ...
    async def _search_warm_storage(self, query: str | None, atom_type: AtomType | None, tags: set[str] | None, min_confidence: float, limit: int) -> list[KnowledgeAtom]:
        results = []
