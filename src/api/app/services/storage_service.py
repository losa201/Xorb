...
class StorageService:
    ...
    async def create_upload_url(
        ...
        # Generate unique identifier hash (will be updated with actual file hash after upload)
        temp_hash = await self._generate_upload_identifier(response.file_id, filename, tenant_id)
    ...
    async def _generate_upload_identifier(self, file_id: str, filename: str, tenant_id: UUID) -> str:
        """Generate secure upload identifier hash"""
        try:
            # Create unique identifier using multiple components for security
            identifier_components = [
                str(file_id),
                filename,
                str(tenant_id),
                datetime.utcnow().isoformat(),
                str(uuid4())  # Add randomness for uniqueness
            ]
            
            # Generate SHA256 hash of combined components
            combined_data = "|".join(identifier_components).encode('utf-8')
            upload_hash = hashlib.sha256(combined_data).hexdigest()
            
            logger.debug(f"Generated upload identifier for file {filename}: {upload_hash[:12]}...")
            return upload_hash
            
        except Exception as e:
            logger.error(f"Failed to generate upload identifier: {e}")
            # Fallback to simple hash
            return hashlib.sha256(f"{file_id}{filename}".encode()).hexdigest()
    
    async def update_file_hash(self, evidence_id: UUID, actual_file_hash: str) -> bool:
        """Update evidence record with actual file hash after upload"""
        try:
            evidence = await self.evidence_repo.get_by_id(evidence_id)
            if not evidence:
                logger.error(f"Evidence {evidence_id} not found for hash update")
                return False
            
            # Update with actual SHA256 hash
            evidence.sha256_hash = actual_file_hash
            evidence.updated_at = datetime.utcnow()
            
            await self.evidence_repo.update(evidence)
            
            logger.info(f"Updated evidence {evidence_id} with actual file hash: {actual_file_hash[:12]}...")
            return True
            
        except Exception as e:
            logger.error(f"Failed to update file hash for evidence {evidence_id}: {e}")
            return False
    
    async def verify_file_integrity(self, evidence_id: UUID, uploaded_hash: str) -> bool:
        ...
        try:
            ...
            return evidence.sha256_hash == uploaded_hash
            
        except Exception as e:
            logger.error(f"File integrity verification failed for {evidence_id}: {e}")
            return False