...
class EcosystemIntegrationAgent:
    ...
    async def _parse_stix_pattern(self, pattern: str) -> str | None:
    ...
    async def _determine_indicator_type(self, value: str, pattern: str) -> IndicatorType:
        ...
        if "file:hashes" in pattern:
            ...
            import re
            if re.match(r'^[0-9a-fA-F]{32}$', value) or re.match(r'^[0-9a-fA-F]{40}$', value) or re.match(r'^[0-9a-fA-F]{64}$', value):  # MD5
                return IndicatorType.FILE_HASH
    ...
    def _map_misp_type_to_indicator_type(self, misp_type: str) -> IndicatorType | None:
    ...
    def _get_eligible_partners_for_tlp(self, tlp_marking: str) -> list[str]:
        ...
        for partner_id, partner in self.partners.items():
            ...
            if tlp_marking in allowed_tlp:
                ...
                if 'all' in sharing_policy or 'partners' in sharing_policy and partner.partner_type in [IntegrationType.CERT, IntegrationType.ISAC] or 'community' in sharing_policy and partner.partner_type == IntegrationType.COMMUNITY:
    ...
    async def _get_taxii_collection_url(self, partner: IntegrationPartner, collection_type: str) -> str | None:
