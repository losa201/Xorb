...
class XORBRefactoredDemo:
    ...
    def test_core_domain(self):
        ...
        try:
            from domains.core import config, Agent, Campaign, AgentType, CampaignStatus
            from domains.core.exceptions import XORBError, SecurityError
            
            # Test configuration system
            self.print_success("Configuration loaded", f"Environment: {config.environment}")
            self.print_success("Database config", f"PostgreSQL URL configured")
    ...
    def test_agent_registry(self):
        ...
        try:
            from domains.agents.registry import registry, AgentRegistry, AgentCapability
    ...
    def test_security_framework(self):
        ...
        try:
            from domains.security.jwt import JWTManager, jwt_manager
    ...
    def test_async_utilities(self):
        ...
        try:
            from domains.utils.async_helpers import AsyncPool, AsyncBatch, CircuitBreaker
            ...
            if legacy_path.exists():
                ...
                for file_path in important_files:
                    if Path(file_path).exists():
                        self.print_success(f"Legacy file", file_path)
                    else:
                        self.print_warning(f"Missing legacy file", file_path)
