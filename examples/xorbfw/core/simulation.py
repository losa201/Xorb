...
class WorldState:
        ...
        for update in updates:
            if update["type"] == "node_add":
                self.topology_graph.add_node(
                    update["node_id"], **update.get("attributes", {})
                )
            elif update["type"] == "edge_add":
                self.topology_graph.add_edge(
                    update["source"], update["target"], **update.get("attributes", {})
                )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "topology": {
                "nodes": [
                    {"id": n, **self.topology_graph.nodes[n]}
                    for n in self.topology_graph.nodes()
                ],
                "edges": [
                    {"source": u, "target": v, **self.topology_graph.edges[u, v]}
...
class SimulationEngine:
    ...
    def _generate_default_topology(self) -> None:
        ...
        self.world_state.topology_graph.add_node(
            "central_hub", type="server", critical=True
        )
    ...
    def _update_threat_zones(self) -> None:
        ...
        red_agents = {
            aid: state
            for aid, state in self.world_state.agent_states.items()
            if state["type"] == "red"
        }
        blue_agents = {
            aid: state
                ...
                self.world_state.threat_zones.append(
                    {
                        ...
                        "intensity": threat_level
                        * 0.5,  # Lower intensity for non-encounter activities
                        "duration": 5,
                    }
                )
    ...
    def _update_comms(self) -> None:
        ...
        active_agents = sum(
            1
            for state in self.world_state.agent_states.values()
    ...
    def _handle_exfiltrate_action(self, agent_id: str, action: Dict[str, Any]) -> None:
        ...
        if target in self.world_state.topology_graph.nodes():
            ...
            available_bandwidth = (
                comms["bandwidth"]["available"] - comms["bandwidth"]["used"]
            )
    ...
    def _handle_stealth_action(self, agent_id: str, action: Dict[str, Any]) -> None:
        ...
        self.world_state.agent_states[agent_id]["stealth_level"] = min(
            1.0, self.world_state.agent_states[agent_id].get("stealth_level", 0.5) + 0.1
        )
    ...
    def load_state(self, path: str) -> None:
        ...
        with open(path, "r") as f:
            ...
            for edge in state_data["topology"]["edges"]:
                self.world_state.topology_graph.add_edge(
...
class BaseAgent(ABC):
    ...
    def __init__(
        self, agent_id: str, initial_position: str, stealth_level: float = 0.5
    ):
    ...
    def get_resources(self) -> Dict[str, float]:
        ...
        return {
            "cpu": random.uniform(
                0.7, 1.0
            ),  # Agents have varying resource availability
    ...
    def is_action_valid(self, action: Dict[str, Any], world_state: WorldState) -> bool:
        ...
        if action_type == "move":
            ...
            target = action.get("target")
            return (
                target in world_state.topology_graph.nodes()
                and len(
                    world_state.topology_graph.nodes()[target].get(
                        "vulnerabilities", []
                    )
                )
                > 0
            ...
            return (
                target in world_state.topology_graph.nodes()
                and world_state.topology_graph.nodes()[target].get("data_size", 0) > 0
            )
    ...
    def calculate_action_cost(self, action: Dict[str, Any]) -> Dict[str, float]:
        ...
        costs = {"cpu": 0.1, "memory": 0.05, "bandwidth": 0.05}
    ...
    def has_sufficient_resources(
        self, action: Dict[str, Any], world_state: WorldState
    ) -> bool:
        ...
        return all(
            resources[res_type] >= cost for res_type, cost in action_cost.items()
        )
    ...
    def _is_in_zone(self, position: str, zone: Dict[str, Any]) -> bool:
        ...
        return (
            position == zone["center"] or random.random() < 0.3
        )  # Simplified for demo
    ...
    def get_nearby_agents(self, world_state: WorldState) -> List[str]:
        ...
        return [
            aid
            for aid, state in world_state.agent_states.items()
