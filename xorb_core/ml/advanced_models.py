...
class ThreatClassificationModel:
    ...
    def train(self, training_data: list[dict], labels: list[str]) -> TrainingResult:
        ...
        feature_importance = dict(zip(feature_names, self.classifier.feature_importances_, strict=False))
    ...
    def predict(self, incident_data: dict) -> PredictionResult:
        ...
        return PredictionResult(
            ...
            timestamp=datetime.now(),
            metadata={
                "all_probabilities": dict(zip(self.threat_classes, probabilities.tolist(), strict=False)),
...
class RiskScoringModel:
    ...
    def _get_time_risk_multiplier(self, current_time: str | None) -> float:
...
class MLModelManager:
    ...
    async def predict(self, model_type: ModelType, input_data: Any, **kwargs) -> PredictionResult:
        ...
        if model_type == ModelType.ANOMALY_DETECTION or model_type == ModelType.THREAT_CLASSIFICATION:
