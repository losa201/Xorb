...
class NATSJetStreamClient:
    ...
    def __init__(self, nats_servers: List[str], redis_client: aioredis.Redis, tenant_id: str):
    ...
    async def _publish_with_idempotency(self, message: Message) -> str:
        ...
        if await self.redis_client.exists(dedup_key):
            logger.info(f"Message {message.idempotency_key} already published (deduplication)")
        ...
        try:
            ...
            logger.info(f"Published message {message.idempotency_key} to {message.subject}")
            return ack.seq
    ...
    async def _handle_message_with_fencing(self, msg, handler: Callable) -> None:
        ...
        try:
            ...
            fence_acquired = await self.redis_client.set(
                fence_key, "locked", nx=True, ex=300  # 5 minute fence
            )
...
async def example_usage():
    ...
    # Initialize Redis for idempotency and fencing
