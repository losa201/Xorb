...
class AdvancedRLAgent(BaseAgent):
    ...
    def meta_learn(self, experiences):
        ...
        meta_inputs = np.concatenate(
            [states, np.eye(self.action_size)[actions], rewards.reshape(-1, 1)], axis=1
        )
    ...
    def adapt_policy(self, context):
        ...
        adapted_weights = []
        for weight, meta_weight in zip(
            self.model.get_weights(), self.meta_model.get_weights()
        ):
    ...
    def get_reward(self, state, action, next_state, mission_context):
        ...
        mission_progress = self._calculate_mission_progress(
            state, next_state, mission_context
        )
