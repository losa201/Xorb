...
class BaseAgent(SimulationObject):
    ...
    def _is_visible(self, obj: SimulationObject, world_state: WorldState) -> bool:
        ...
        obj_position = np.array(
            obj.position if hasattr(obj, "position") else [0.0, 0.0]
        )
    ...
    def execute(self, action: AgentAction, world_state: WorldState) -> Dict[str, Any]:
        ...
        result = {"success": True, "reward": 0.0, "done": False}
...
class RuleBasedPolicy(AgentPolicy):
    ...
    def _rule_condition_met(
        self, rule: Dict[str, Any], state: AgentState, perception: Perception
    ) -> bool:
...
class Mission:
    ...
    def __init__(
        self, mission_id: str, mission_type: str, objectives: List[Dict[str, Any]]
    ):
        self.id = mission_id
...
class ResourceManager:
    ...
    def get_resources_in_range(
        self, location: List[float], radius: float
    ) -> List[Resource]:
    ...
    def get_resource_usage(self) -> Dict[str, Dict[str, float]]:
        ...
        for resource_id, resource in self.resources.items():
            stats[resource_id] = {
                "type": resource.type,
                "usage": resource.amount / resource.max_capacity
...
def create_rl_agent(
    agent_id: str, agent_type: str, observation_space, action_space
) -> BaseAgent:
...
def create_mission_planner() -> MissionPlanner:
    ...
    planner.register_template("default_attack", {"type": "attack", "objectives": []})
    planner.register_template("default_defense", {"type": "defense", "objectives": []})
    return planner
