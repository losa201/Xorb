...
class AppSettings(BaseSettings):
    ...
    allowed_file_extensions: str = Field(default=".pdf,.txt,.json,.xml,.csv", env="ALLOWED_FILE_EXTENSIONS")
    ...
    def validate_jwt_secret(cls, v, values):
        ...
        if v.lower() in [default.lower() for default in insecure_defaults]:
            raise ValueError("JWT secret cannot be a common default value")
        ...
        if environment == "production":
            ...
            if unique_chars < 16:
                raise ValueError("JWT secret must contain at least 16 unique characters in production")
    ...
    model_config = SettingsConfigDict(
        env_file=None,  # Disable .env file for now to test environment variables
